---
type: index
id: CTX-000
phase: phase-10-deployment
status: complete
created: 2026-08-21
updated: 2026-08-21
related:
  - "[[01-current-state]]"
  - "[[02-active-work]]"
  - "[[03-next-actions]]"
---

# SpecSathi Context Vault

This vault is the recoverable working memory for the offline-first procurement-control demo. Start every resumed session with [[01-current-state]], then read [[02-active-work]] and [[03-next-actions]].

Current phase: [[phases/phase-10-deployment]].

Key decisions: [[decisions/DEC-001-initial-architecture]]. Resume with [[01-current-state]] and run the documented local checks before any further change.
