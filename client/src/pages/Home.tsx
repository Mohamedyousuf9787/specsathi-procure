...
import GenericProcurementWorkspace from "@/components/GenericProcurementWorkspace";
...
export default function Home() {
  const [view, setView] = useState<"intake" | "review" | "generic-workspace" | "legacy-workspace">("intake");
  ...
  const reviewBrief = () => {
    const result = parseBuyingBrief(brief);
    setValidation(result);
    if (result.status === "valid") setView("review");
  };
  ...
  const startProcurement = async () => {
    if (!validation?.normalizedBrief) return;
    const session = await runGenericProcurement(validation.normalizedBrief);
  ...
  return <div className="min-h-screen bg-[#f6f5f4] text-[#31302e]">
    ...
    {(view === "intake" || view === "review") && <><section className="relative overflow-hidden bg-[#213183] text-white"><div className="mx-auto grid min-h-[230px] max-w-[1360px] items-center px-4 py-8 sm:px-6 lg:grid-cols-[minmax(0,1fr)_410px] lg:px-8"><div className="relative z-10 max-w-[720px]"><span className="inline-flex items-center gap-2 rounded-full bg-white px-3 py-1.5 text-[11px] font-semibold uppercase tracking-[0.07em] text-[#0075de]"><ShieldCheck className="h-3.5 w-3.5" />Autonomous procurement control</span><h1 className="mt-4 text-[38px] font-bold leading-[.98] tracking-[-1.5px] sm:text-[50px]">Record the purchase.<br />Hold the boundary.</h1><p className="mt-3 max-w-[590px] text-[14px] leading-5 text-white/75">SpecSathi normalizes the brief, compares two vendor records, and pauses exactly where authorization requires a human decision.</p></div><div className="relative hidden lg:block"><img src={NIGHT_DESK} alt="Decorative receipt, approval, vendor ticket, and delivery evidence stickers" className="h-auto w-full" /><div className="absolute bottom-3 left-3 flex flex-wrap gap-1.5"><span className="rounded bg-white/95 px-2 py-1 text-[9px] font-bold uppercase tracking-[0.06em] text-[#213183]">Receipt verified</span><span className="rounded bg-white/95 px-2 py-1 text-[9px] font-bold uppercase tracking-[0.06em] text-[#213183]">Approval held</span></div></div></div></section><main className="mx-auto max-w-[1180px] px-4 py-8 sm:px-6 lg:px-8 lg:py-12">
      {view === "intake" && <section className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]"><div className="rounded-xl border border-[#e6e6e6] bg-white p-6 shadow-[0_1px_1px_rgba(0,0,0,.02),0_8px_20px_rgba(0,0,0,.035)] sm:p-8"><div className="flex items-start gap-3"><span className="grid h-10 w-10 place-items-center rounded-lg bg-[#d6b6f6] text-[#391c57]"><FileText className="h-5 w-5" /></span><div><p className="text-[12px] font-semibold uppercase tracking-[0.07em] text-[#0075de]">Procurement control record</p><h2 className="mt-1 text-[28px] font-bold tracking-[-0.7px] text-black">Name the online purchase.</h2></div></div><label htmlFor="generic-brief" className="mt-7 block text-[13px] font-semibold text-[#31302e]">Buying brief</label><textarea id="generic-brief" value={brief} onChange={(event) => { setBrief(event.target.value); setValidation(null); }} placeholder="Purchase 10 laptops with 16 GB RAM and 512 GB SSD under ₹45,000 each within 5 days." className="mt-2 min-h-40 w-full resize-y rounded border border-[#ddd] bg-white p-3 text-[14px] leading-6 text-[#31302e] outline-none ring-[#0075de] placeholder:text-[#a39e98] focus:ring-2" /><div className="mt-3 grid gap-px border-y border-[#e6e6e6] py-2 text-[11px] font-semibold uppercase tracking-[0.06em] text-[#615d59] sm:grid-cols-3"><span className="py-1">01 · Record constraints</span><span className="py-1">02 · Compare evidence</span><span className="py-1">03 · Hold exceptions</span></div>{validation && validation.status !== "valid" && <div role="status" className="mt-3 rounded-lg border border-[#dd5b00]/25 bg-[#fff5ed] p-3 text-[12px] leading-5 text-[#9a3b00]"><p className="font-bold">{validation.status === "invalid" ? "The control record needs correction before a safe search." : "A concise clarification is needed before a safe search."}</p><p className="mt-1">{validation.conflicts[0]?.message ?? validation.clarifyingQuestions[0]}</p></div>}<div className="mt-5 flex flex-wrap items-center justify-between gap-3"><p className="inline-flex items-center gap-2 text-[12px] leading-5 text-[#615d59]"><ShieldCheck className="h-4 w-4 text-[#1aae39]" />Local evidence only. No API key or connection required.</p><div className="flex flex-wrap gap-2"><button onClick={loadLaptopDemo} className="min-h-10 rounded-lg border border-[#e6e6e6] bg-white px-4 text-[13px] font-semibold text-[#31302e] transition hover:border-[#a39e98] active:scale-[.97]">Load laptop demo</button><button onClick={loadMultiDemo} className="min-h-10 rounded-lg border border-[#e6e6e6] bg-white px-4 text-[13px] font-semibold text-[#31302e] transition hover:border-[#a39e98] active:scale-[.97]">Load multi-item demo</button><button onClick={reviewBrief} className="inline-flex min-h-10 items-center gap-2 rounded-full bg-[#0075de] px-4 text-[13px] font-semibold text-white shadow-[0_5px_13px_rgba(0,117,222,.18)] transition hover:bg-[#005bab] active:scale-[.97]">Inspect policy record <ArrowRight className="h-4 w-4" /></button></div></div></div><aside className="rounded-xl border border-[#e6e6e6] bg-white p-6"><p className="text-[12px] font-semibold uppercase tracking-[0.07em] text-[#615d59]">Control sequence</p><div className="mt-5 space-y-5">{[[Search, "Record requirements", "Product, quantity, budget, delivery, and authorization become an explicit policy record."], [ShieldCheck, "Verify vendor evidence", "Vendor A and Vendor B are normalized before any recommendation is made."], [FileClock, "Write the decision trail", "Every search, boundary, approval, and simulated order remains inspectable."]].map(([Icon, title, copy]) => { const Symbol = Icon as typeof Search; return <div key={title as string} className="flex gap-3"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-md bg-[#f6f5f4] text-[#0075de]"><Symbol className="h-4 w-4" /></span><div><h3 className="text-[14px] font-bold text-[#31302e]">{title as string}</h3><p className="mt-1 text-[12px] leading-5 text-[#615d59]">{copy as string}</p></div></div>; })}</div><div className="mt-7 border-y border-[#e6e6e6] py-4"><p className="text-[11px] font-semibold uppercase tracking-[0.07em] text-[#615d59]">Policy tape</p><p className="mt-2 text-[12px] font-bold leading-5 text-[#31302e]">Hard requirement → two-source evidence → authorization check</p></div><div className="mt-5"><p className="text-[11px] font-semibold uppercase tracking-[0.07em] text-[#a39e98]">Simulation notice</p><p className="mt-2 text-[13px] font-semibold leading-5 text-[#31302e]">SIMULATED PURCHASE — NO REAL PAYMENT</p></div></aside></section>}
      {view === "review" && normalized && <section><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-[12px] font-semibold uppercase tracking-[0.07em] text-[#0075de]">Step 1 of 3 / confirm requirements</p><h2 className="mt-1 text-[34px] font-bold tracking-[-1px] capitalize text-black">The agent identified a {normalized.productCategory} request.</h2><p className="mt-2 max-w-[660px] text-[14px] leading-6 text-[#615d59]">Confirm the structured record before SpecSathi searches local vendors. Requirements that cannot be verified will block auto-purchase rather than be assumed.</p></div><button onClick={() => setView("intake")} className="min-h-10 rounded-lg border border-[#e6e6e6] bg-white px-4 text-[13px] font-semibold text-[#31302e] hover:border-[#a39e98]">Edit brief</button></div><div className="mt-7 grid gap-5 md:grid-cols-[1.3fr_.7fr]"><section className="rounded-xl border border-[#e6e6e6] bg-white p-6 shadow-[0_1px_1px_rgba(0,0,0,.02),0_8px_20px_rgba(0,0,0,.025)]"><p className="text-[12px] font-semibold uppercase tracking-[0.07em] text-[#0075de]">Normalized requirement record</p><div className="mt-4 grid gap-4 sm:grid-cols-3"><div><p className="text-[11px] text-[#a39e98]">Product</p><p className="mt-1 text-[16px] font-bold capitalize text-black">{normalized.productCategory}</p></div><div><p className="text-[11px] text-[#a39e98]">Quantity</p><p className="mt-1 text-[16px] font-bold text-black">{normalized.quantity} units</p></div><div><p className="text-[11px] text-[#a39e98]">Authorization</p><p className="mt-1 text-[16px] font-bold tabular-nums text-black">{normalized.authorizationLimitInr ? `${rupees(normalized.authorizationLimitInr)} / unit` : "Not set"}</p></div></div><div className="mt-5 border-t border-[#e6e6e6] pt-5"><p className="text-[11px] font-semibold uppercase tracking-[0.07em] text-[#615d59]">Hard requirements</p><div className="mt-2 flex flex-wrap gap-2">{normalized.hardRequirements.length ? normalized.hardRequirements.map((requirement) => <span key={requirement.key} className="rounded-md bg-[#f6f5f4] px-3 py-1.5 text-[12px] font-semibold text-[#31302e]">{requirement.label} {requirement.operator.replaceAll("_", " ")} {String(requirement.value)}{requirement.unit ? ` ${requirement.unit}` : ""}</span>) : <span className="text-[13px] text-[#a39e98]">No product-specific requirement identified.</span>}</div></div>{validation?.warnings.length ? <div className="mt-5 rounded-lg bg-[#edf6ff] p-3 text-[12px] leading-5 text-[#005bab]"><p className="font-bold">Checks to keep visible</p><p className="mt-1">{validation.warnings[0]}</p></div> : null}</section><aside className="rounded-xl border border-[#e6e6e6] bg-white p-6"><p className="text-[12px] font-semibold uppercase tracking-[0.07em] text-[#615d59]">Search plan</p><p className="mt-2 text-[14px] font-bold text-[#31302e]">Two local sources</p><p className="mt-1 text-[12px] leading-5 text-[#615d59]">Vendor A and Vendor B will be normalized into one comparison record. Unknown categories fail safely with no invented offers.</p><p className="mt-5 text-[11px] font-semibold uppercase tracking-[0.07em] text-[#a39e98]">Confidence</p><p className="mt-1 text-[24px] font-bold tracking-[-0.6px] text-black">{Math.round(normalized.confidence * 100)}%</p></aside></div><div className="mt-7 flex flex-wrap items-center justify-between gap-4 rounded-xl border border-[#e6e6e6] bg-white p-5 shadow-[0_1px_1px_rgba(0,0,0,.02),0_8px_20px_rgba(0,0,0,.025)]"><p className="text-[13px] leading-5 text-[#615d59]"><strong className="text-[#31302e]">Policy rule:</strong> every hard requirement must be verifiable, and an over-limit offer must pause for explicit approval.</p><button onClick={startProcurement} className="inline-flex min-h-11 items-center gap-2 rounded-full bg-[#0075de] px-5 text-[14px] font-semibold text-white shadow-[0_5px_13px_rgba(0,117,222,.18)] transition hover:bg-[#005bab] active:scale-[.97]">Start procurement <ArrowRight className="h-4 w-4" /></button></div></section>}
    </main></>}
    {view === "generic-workspace" && genericSession && <main className="mx-auto max-w-[1360px] px-4 sm:px-6 lg:px-8"><GenericProcurementWorkspace session={genericSession} onSessionChange={setGenericSession} onNewBrief={newBrief} onLoadMultiDemo={loadMultiDemo} /></main>}
