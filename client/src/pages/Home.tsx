...
export default function Home() {
  ...
  const nlpExtraction = trpc.nlp.extractBrief.useMutation({
    onSuccess: (result) => {
      if (result.source !== "llm" || !result.candidate) {
  ...
  const startProcurement = async () => {
    ...
    const query = `${validation.normalizedBrief.productCategory} ${validation.normalizedBrief.hardRequirements.map((requirement) => `${requirement.label} ${String(requirement.value)}`).join(" ")} vendor price India`;
  ...
  return <div className="min-h-screen bg-[#f6f5f4] text-[#31302e]">
    ...
    {view === "generic-workspace" && genericSession && <main className="mx-auto max-w-[1360px] px-4 sm:px-6 lg:px-8"><GenericProcurementWorkspace session={genericSession} onSessionChange={setGenericSession} onNewBrief={newBrief} onLoadMultiDemo={loadMultiDemo} liveEvidence={liveEvidence} auditPersistence={auditPersistence} /></main>}
