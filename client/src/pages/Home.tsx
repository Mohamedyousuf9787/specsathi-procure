...
import ProcurementMenu, { type ProcurementMenuItem } from "@/components/ProcurementMenu";
import ProcurementInfoPanel from "@/components/ProcurementInfoPanel";
...
type InfoPanel = "history" | "how" | "status" | "about" | null;
...
async function downloadFinanceAuditPdf(session: ProcurementSession) {
  const [{ jsPDF }, { buildFinanceAuditPdfDocument }] = await Promise.all([
    import("jspdf"),
    import("@/domain/finance-audit-pdf"),
  ]);
  const documentModel = buildFinanceAuditPdfDocument(session);
  ...
  for (const line of documentModel.lines) {
    ...
    const requiredHeight = Math.max(14, wrapped.length * 14) + 4;
    if (y + requiredHeight > 842 - bottom) {
      pdf.addPage();
      y = 54;
    }
  ...
  pdf.save(documentModel.filename);
}
    ...
    <div className="min-h-screen bg-[#f6f5f4] text-[#31302e]">
      <header className="sticky top-0 z-30 border-b border-[#e6e6e6] bg-white/95 backdrop-blur-md">
        <div className="mx-auto flex min-h-16 max-w-[1360px] items-center justify-between px-4 sm:px-6 lg:px-8">
          <button onClick={reset} className="flex items-center gap-2.5 text-left"><img src={BRAND_MARK} alt="Specanic" className="h-10 w-10" /><span><span className="block text-[18px] font-bold tracking-[-0.65px] text-black">Spec<span className="text-[#0075de]">anic</span></span><span className="mt-0.5 flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-[0.16em] text-[#615d59]"><i aria-hidden="true" className="h-px w-3 bg-[#0075de]" />Procure</span></span></button>
...
export default function Home() {
  ...
  const productSearch = trpc.products.search.useMutation({
    onSuccess: (result, variables) => {
      ...
      setGenericSession(current => !current || current.brief.id !== lastConfirmedBriefRef.current?.id ? current : recordMarketplaceSearchOutcome(current, { status: outcome.status, listingCount: outcome.listings.length, unverifiedListingCount: outcome.listings.filter(listing => listing.policy === "unverified").length, message: outcome.message }));
    },
    onError: () => {
  ...
  const handleMenuSelect = (item: ProcurementMenuItem) => {
    setMenuOpen(false);
    if (item === "home" || item === "new") return newBrief();
    setInfoPanel(item);
  };
  const reviewBrief = () => {
    if (!brief.trim()) {
  ...
  useEffect(() => {
    ...
    if (demo === "mouse") loadCuratedDemo(mouseDemoBrief);
    if (demo === "gpu") loadCuratedDemo(gpuDemoBrief);
  }, []);
  const loadMultiDemo = () => {
  ...
  const startProcurement = async (confirmedBrief = validation?.normalizedBrief, policyAgreement?: PolicyAgreement) => {
    ...
    productSearch.mutate({ query, category: confirmedBrief.productCategory, maxUnitPriceInr: confirmedBrief.maxUnitPriceInr, authorizationLimitInr: confirmedBrief.authorizationLimitInr, hardRequirements: confirmedBrief.hardRequirements.map(requirement => ({ key: requirement.key, label: requirement.label, operator: requirement.operator === "preferred" ? "contains" : requirement.operator, value: requirement.value, unit: requirement.unit })) });
    setView("generic-workspace");
  };
  ...
  return <div className="min-h-screen bg-[#f6f5f4] text-[#31302e]">
    ...
    <footer className="border-t border-[#e6e6e6] bg-[#f6f5f4]"><div className="mx-auto flex max-w-[1360px] flex-wrap items-center justify-between gap-3 px-4 py-7 text-[12px] text-[#615d59] sm:px-6 lg:px-8"><p>Specanic · Category-agnostic mechanism with a laptop-focused demo.</p><p className="inline-flex items-center gap-2"><MapPin className="h-3.5 w-3.5 text-[#0075de]" />Local demo data · Simulated vendors · No real payment</p></div></footer>
  </div>;
}
