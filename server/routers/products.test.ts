...
import { clearProductSearchCacheForTests, normalizeFastMarketplaceSpecifications, normalizeShoppingResults, productsRouter } from "./products";

describe("shopping product normalization", () => {
  afterEach(() => {
    clearProductSearchCacheForTests();
    vi.unstubAllEnvs();
    vi.unstubAllGlobals();
  });
  ...
  it("deduplicates concurrent identical live searches and serves the immediate repeat from the bounded cache", async () => {
    ...
    const caller = productsRouter.createCaller({ user: undefined } as never);
    const input = { query: "business laptop", category: "laptop", maxUnitPriceInr: 45_000, authorizationLimitInr: 40_000 };
    const [first, second] = await Promise.all([caller.search(input), caller.search(input)]);
    const third = await caller.search(input);
